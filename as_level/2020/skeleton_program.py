  ...
  try:
    for Row in range(Header.Height):
      for Column in range(Header.Width):
        ...
        plaintext += FindSecretChar(PixelValue, key)
  except:
...
def MirrorImage(Grid, Header):
  ...
  # use the header width and height to take parts of the grid and reverse it.
  for row in range(Header.Height):
    line = Grid[row][:Header.Width]
    white_space = Grid[row][Header.Width:]
    line.reverse()
    Grid[row] = line + white_space
  DisplayImage(Grid, Header)

def FindSecretChar(PixelValue, Key):
  # code which is value - key
  # if the code 0 then return empty space
  # if the code 0 - 26 return the ascii char
  # return blank
  code = PixelValue - Key
  if code == 0:
    return " "
...
def CompressFile():
    ...
    with open(fname + ".txt", "r") as f:
      ...
      header_linec = header_line[:-2] + "C"
      data = f.readline()

    with open("CMP" + fname + ".txt", "w") as fc:
      fc.write(header_linec + "/n")
      index = 1
      count = 1
      while index < len(data):
        if data[index] == data[index-1]:
          count+=1
          if index == len(data) -1:
            fc.write(str(count) + "," + data[index])
          else:
            fc.write(str(count) + ",", data[index-1] + "/n")
            count = 1
          index += 1
