...
def ProtectedCell(Puzzle, CellInfo):
    for cell in Puzzle:
        if cell[:-1] == CellInfo[:-1]:
            return True
    return False

def SubGrid(PuzzleGrid, Row, Column):
    subgrid = [] 
    if Row <= 3:
        for i in range(1,4):
            if Column <= 3:
                for j in range(1,4):
                    subgrid.append(PuzzleGrid[i][j])
            elif Column <= 6:
                for j in range(4,7):
                    subgrid.append(PuzzleGrid[i][j])
            else:
...
def DuplicateDigit(PuzzleGrid, Row, Column, Digit):
    print("log:", Row, Column, Digit)
    print("row:", PuzzleGrid[Row])
    if Digit in PuzzleGrid[Row]:
        return True
    for row in PuzzleGrid:
        print("column:", row[Column])
        if Digit == row[Column]:
            return True
    for cell in SubGrid(PuzzleGrid, Row, Column):
        if Digit == cell:
            return True
    return False

def ClearEntries(num, Answer, PuzzleGrid):
    while True:
        if num >= 0:
            break
        moves = int(Answer[2])
        ...
        move = Answer[moves + 2]
        Answer[moves + 2] = ''
        PuzzleGrid[int(move[0])][int(move[1])] = ' '
        Answer[2] = str(moves - 1)
        num += 1
...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer):
  ...
  if PuzzleGrid[0][0] != 'X':
    ...
    while CellInfo != EMPTY_STRING:
      InputError = False
      ProtectedCellError = False
      if CellInfo[0] == '-':
          try:
            num = int(CellInfo)
            ClearEntries(num, Answer, PuzzleGrid)
            DisplayGrid(PuzzleGrid)
          except ValueError:
            print("Invalid input")
      else:
        if len(CellInfo) != 3:
            InputError = True
        elif ProtectedCell(Puzzle, CellInfo):
            ProtectedCellError = True
        else:
            Digit = CellInfo[2]
            try:
                Row = int(CellInfo[0])
        ...
        if InputError:
            print("Invalid input")
        elif ProtectedCellError:
            print("Sorry, protected cell")
        elif DuplicateDigit(PuzzleGrid, Row, Column, Digit):
