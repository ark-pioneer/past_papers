...
class Company:
  ...
  def ProcessDayEnd(self):
    ...
    self._Balance -= self._LoanBalance * self._InterestRate
...
class Simulation:
  ...
  def AddCompany(self):
    ...
    while True:
      ...
      if CompanyName == "":
        print("invalid, you must enter a name")
      elif self.GetIndexOfCompany(CompanyName) != -1:
        print("invalid, name must be unique")
      else:
        break
  ...
  def ModifyCompany(self, Index):
    ...
    if Choice == "2" or Choice == "3":
      ...
      self._Companies[Index].GetLoan(rate)
    elif Choice == "5":
      payment_amount =  int(input("Enter amount to pay back: "))
      self._Companies[Index].PayLoan(payment_amount)
    print()
