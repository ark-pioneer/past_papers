...
class Breakthrough():
    ...
    def __GetCardChoice(self):
        ...
        while True:
            try:
                hand_size = self.__Hand.GetNumberOfCards()
                Choice = int(input(f"Enter a number between 1 and {hand_size} to specify card to use:> "))
                if Choice not in range(1, hand_size+1):
                    raise ValueError
                break
            except:
                print(f"You must enter an integer between 1 and {hand_size}")
        return Choice
