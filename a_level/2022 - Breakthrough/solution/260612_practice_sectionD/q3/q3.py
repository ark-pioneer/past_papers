...
class Breakthrough():
    ...
    def __CheckIfLockChallengeMet(self):
        SequenceAsString = ""
        for Count in range(self.__Sequence.GetNumberOfCards() - 1, max(0, self.__Sequence.GetNumberOfCards() - self.__CurrentLock.GetLongestChallengeLength()) -1, -1):
...
class Lock():
    ...
    def GetLongestChallengeLength(self):
        return max(len(c.GetCondition()) for c in self._Challenges)
