...
class Breakthrough():
        ...
        if len(LineFromFile) > 0:
            ...
            for Item in SplitLine:
                ...
                if Item[0: 3] == "Dif":
                    ...
                    CurrentCard = BonusCard(CardNumber)
                    CardCol.AddCard(CurrentCard)
                else:
    ...
    def __GetCardFromDeck(self, CardChoice):
        ...
        while self.__Hand.GetNumberOfCards() < 5 and self.__Deck.GetNumberOfCards() > 0:
            ...
            elif self.__Deck.GetCardDescriptionAt(0) == "Bon":
                print()
                print(f"When refilling your hand, a Bonus card appeared!")
                CurrentCard = self.__Deck.RemoveCard(self.__Deck.GetCardNumberAt(0))
                self.__Score += CurrentCard.GetScore()
                print(f"Score increased by {CurrentCard.GetScore()}")
                self.__Discard.AddCard(CurrentCard)
                print("Bonus Card moved to Discard")
            else:
                self.__MoveCard(self.__Deck, self.__Hand, self.__Deck.GetCardNumberAt(0))
    ...
    def __AddBonusCardsToDeck(self):
        for Count in range(2):
            self.__Deck.AddCard(BonusCard())
