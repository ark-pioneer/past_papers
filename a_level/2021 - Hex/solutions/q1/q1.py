...
class Ranger(Piece):
  def __init__(self, Player1):
    super(Ranger, self).__init__(Player1)
    self._PieceType = "R"
  ...
  def CheckMoveIsValid(self, DistanceBetweenTiles, StartTerrain, EndTerrain):
    if StartTerrain == "#" and EndTerrain == "#":
      ...
      return super(Ranger, self).CheckMoveIsValid(DistanceBetweenTiles, StartTerrain, EndTerrain)
...
class HexGrid:
  ...
  def AddPiece(self, BelongsToPlayer1, TypeOfPiece, Location):
    if TypeOfPiece == "Baron":
      ...
      NewPiece = Ranger(BelongsToPlayer1)
    else:
  ...
  try:
    with open(FileName) as f:
      ...
      while LineFromFile != "":
        ...
        Items = LineFromFile.split(",")
        if Items[0] == "1":
          print(Items[0], 1, 'true')
          Grid.AddPiece(True, Items[1], int(Items[2]))
        else:
          print(Items[0], 1, 'false')
          print(Items)
